...
namespace SPID.AspNetCore.Authentication.Helpers
{
    public static class SamlHelper
    {
        ...
        private static XmlSerializer responseSerializer = new XmlSerializer(typeof(AuthnRequestType));
        ...
        public static EntityDescriptor DownloadMetadataIDP(this string urlMetadataIdp)
        {
            ...
            try
            {
                ...
                response = (EntityDescriptor)serializer.Deserialize(reader);
            ...
            IdentityProvider identityProvider)
        {
            ...
            BusinessValidation.Argument(destination, string.Format(ErrorLocalization.ParameterCantNullOrEmpty, nameof(destination)));
            ...
            {
                identityProvider.DateTimeFormat = SamlIdentityProviderSettings.DefaultSettings[SamlIdentityProviderSettings.DateTimeFormat];
            }

            if (string.IsNullOrWhiteSpace(identityProvider.NowDelta))
            {
                identityProvider.NowDelta = SamlIdentityProviderSettings.DefaultSettings[SamlIdentityProviderSettings.NowDelta];
            }

            if (string.IsNullOrWhiteSpace(assertionConsumerServiceIndex))
            {
                assertionConsumerServiceIndex = SamlIdentityProviderSettings.DefaultSettings[SamlIdentityProviderSettings.AssertionConsumerServiceIndex];
            }


            string dateTimeFormat = identityProvider.DateTimeFormat;
            double nowDelta = Convert.ToDouble(identityProvider.NowDelta);
            ushort assertionConsumerServiceIndexShort = Convert.ToUInt16(assertionConsumerServiceIndex);
            ...
            AuthnRequestType authnRequest = new AuthnRequestType
            {
                ...
                AssertionConsumerServiceIndex = assertionConsumerServiceIndexShort,
                AssertionConsumerServiceIndexSpecified = true,
                AttributeConsumingServiceIndex = assertionConsumerServiceIndexShort,
                AttributeConsumingServiceIndexSpecified = true,
                //ProtocolBinding = SamlConst.ProtocolBinding,
            ...
            return (Convert.ToBase64String(Encoding.UTF8.GetBytes("<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + doc.OuterXml), Base64FormattingOptions.None), authnRequest, samlString);
        }
        ...
        public static void ValidateAuthnResponse(this Response response, AuthnRequestType request, EntityDescriptor metadataIdp, bool validationAll)
        {
            // Verify signature
            ...
            if (validationAll)
            {
                listAttribute.Add(SamlConst.digitalAddress);
                listAttribute.Add(SamlConst.name);
                listAttribute.Add(SamlConst.familyName);
                listAttribute.Add(SamlConst.email);
                listAttribute.Add(SamlConst.address);
                listAttribute.Add(SamlConst.companyName);
                listAttribute.Add(SamlConst.countyOfBirth);
                listAttribute.Add(SamlConst.dateOfBirth);
                listAttribute.Add(SamlConst.expirationDate);
                listAttribute.Add(SamlConst.fiscalNumber);
                listAttribute.Add(SamlConst.gender);
                listAttribute.Add(SamlConst.idCard);
                listAttribute.Add(SamlConst.ivaCode);
                listAttribute.Add(SamlConst.mobilePhone);
                listAttribute.Add(SamlConst.placeOfBirth);
                listAttribute.Add(SamlConst.registeredOffice);
                listAttribute.Add(SamlConst.spidCode);
                ...
                attributeName.AddRange(attribute.Where(x => !string.IsNullOrWhiteSpace(x.Name) && !x.Name.StartsWith("urn")).Select(x => x.Name).ToList());
                BusinessValidation.ValidationCondition(() => attributeName.Count() == 0 /*&& attributeFriendlyName.Count() == 0*/, ErrorLocalization.AttributeRequiredNotFound);
                if (attributeName.Count() > 0)
                    BusinessValidation.ValidationCondition(() => attributeName.Any(x => !listAttribute.Contains(x)), ErrorLocalization.AttributeRequiredNotFound);
            }
            else
            {
                ...
                attributeName.AddRange(attribute.Where(x => !string.IsNullOrWhiteSpace(x.FriendlyName)).Select(x => x.FriendlyName).ToList());
                BusinessValidation.ValidationCondition(() => attributeName.Count() == 0 /*&& attributeFriendlyName.Count() == 0*/, ErrorLocalization.AttributeRequiredNotFound);
                if (attributeName.Count() > 0)
                    BusinessValidation.ValidationCondition(() => listAttribute.All(x => !attributeName.Contains(x)), ErrorLocalization.AttributeRequiredNotFound);
            }
            ...
            if (validationAll)
            {
                ...
                var listAuthRefValid = new List<string>
                {
                    SamlConst.SpidL1,
                    SamlConst.SpidL2,
                    SamlConst.SpidL3
